 ...
 */
 ...
 window.location.search.slice(1).split('&').map(pair => pair.split('=')).forEach(pair => {
    browserParams[pair[0]] = pair[1];
 });

 // Note: NEVER use those exact keys in production!
 const PRIVATE_KEY = browserParams.privateKey;
 ...
 setTimeout(() => {
    document.getElementById('serverKey').innerHTML = SERVER_KEY;
    document.getElementById('publicKey').innerHTML = PUBLIC_KEY;
    document.getElementById('privateKey').innerHTML = PRIVATE_KEY;
    document.getElementById('trustedKey').innerHTML = TRUSTED_KEY;
 }, 1000);
